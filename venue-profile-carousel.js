(function(){
  ...
  function applySharpFit(image,media){
    ...
    const update=()=>{
      const iw=Number(image.naturalWidth||0),ih=Number(image.naturalHeight||0);
      const fw=Number(media.clientWidth||0),fh=Number(media.clientHeight||0);
      if(!iw||!ih||!fw||!fh)return;
      const imageRatio=iw/ih,frameRatio=fw/fh;
      const mismatch=Math.abs(imageRatio-frameRatio);
  ...
  async function loadImages(){
    ...
    if(coverImage){
      ...
      applySharpFit(coverImage,document.getElementById("venueProfileMedia"));
    }
  ...
  function installCarousel(image){
    ...
    function show(nextIndex){
      ...
      setTimeout(()=>{
        ...
        const active=nav.querySelector(".smv-cover-dot.active");
    ...
    let autoTimer=null;
    function startAuto(){if(autoTimer||images.length<2)return;autoTimer=setInterval(()=>show(index+1),4200);}
    ...
    media.addEventListener("touchend",e=>{const endX=e.changedTouches[0]?.clientX||0;if(Math.abs(endX-startX)>45)show(endX<startX?index+1:index-1);startAuto();},{passive:true});
  ...
  function installTopQuickEnquiryStyles(){
    ...
    style.textContent=`
      ...
      .venue-profile-hero.smv-hero-with-quick>.venue-profile-media img{height:100%!important;width:100%!important;object-position:center!important;image-rendering:auto!important;filter:none!important}
      .venue-profile-hero.smv-hero-with-quick>.venue-profile-media img.smv-image-cover{object-fit:cover!important}
      .venue-profile-hero.smv-hero-with-quick>.venue-profile-media img.smv-image-contain{object-fit:contain!important}
  ...
  function placeQuickEnquiryBesideImages(){
    ...
    const place=()=>{
      ...
      if(hero&&media&&form){hero.classList.add("smv-hero-with-quick");if(form.parentElement!==hero)media.insertAdjacentElement("afterend",form);const image=document.getElementById("venueProfileImage");if(image)applySharpFit(image,media);return;}
      tries+=1;if(tries<60)setTimeout(place,100);
    };
    place();
  }
  ...
  async function resolveSimilarImage(card){
    ...
    try{
      ...
      let url="";
      if(!root.error){const cover=(root.data||[]).find(row=>/^cover-/i.test(row?.name||""));if(cover)url=publicUrl(`${id}/${cover.name}`);}
      if(!url){const gallery=await client.storage.from(BUCKET).list(`${id}/gallery`,{limit:1,sortBy:{column:"name",order:"asc"}});const first=(gallery.data||[]).find(row=>row?.name&&row.name!==".emptyFolderPlaceholder");if(first)url=publicUrl(`${id}/gallery/${first.name}`);}
      if(url){const name=card.querySelector("h3")?.textContent?.trim()||"Venue";media.innerHTML=`<img src="${url}" alt="${name.replace(/[<>\"]/g,"")} venue" loading="lazy" decoding="async">`;}
    }catch(_){ }
  }

  function watchSimilarVenueImages(){
    const grid=document.getElementById("similarVenuesGrid");if(!grid)return;
    const process=()=>grid.querySelectorAll(".venue-similar-card").forEach(resolveSimilarImage);
    process();const observer=new MutationObserver(process);observer.observe(grid,{childList:true,subtree:true});setTimeout(()=>{process();observer.disconnect();},5000);
  }
