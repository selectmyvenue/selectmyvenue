(function () {
  ...
  function renderHighlights(venue) {
    ...
    byId("venueProfileHighlights").innerHTML = highlights.slice(0, 6).map(([title, text]) =>
      `<div class="venue-highlight"><b>${escapeHtml(title)}</b><span>${escapeHtml(text)}</span></div>`
    ).join("");
  }
  ...
  function renderGallery(name, media) {
    const section = byId("venueProfileGallerySection");
    const grid = byId("venueProfileGallery");
    const videosSection = byId("venueProfileVideosSection");
    const videosGrid = byId("venueProfileVideos");
  ...
  function renderProfile(venue, media) {
    ...
    if (mapUrl) {
      byId("venueProfileMap").href = mapUrl;
      byId("venueProfileMap").hidden = false;
    }

    const featureList = venueFeatures(venue);
    byId("venueProfileFeatures").innerHTML = featureList.length
      ? featureList.map(([icon, label]) => `<div class="venue-profile-feature"><span>${icon}</span><strong>${escapeHtml(label)}</strong></div>`).join("")
      : `<div class="venue-profile-feature"><span>✦</span><strong>Venue details available on request</strong></div>`;

    renderHighlights(venue);
    renderGallery(name, media);
    ...
    setMeta(name, metaDescription, media.cover);
    setStructuredData(venue, structuredImages, description);
  ...
  function openQuoteModal() {
    ...
    const modal = byId("venueQuoteModal");
    if (!modal) return;
    modal.hidden = false;
    document.body.style.overflow = "hidden";
    setTimeout(() => byId("venueQuoteName")?.focus(), 40);
  }
  ...
  async function submitQuote(event) {
    ...
    const status = byId("venueQuoteStatus");
    const button = byId("venueQuoteSubmit");
    ...
    if (!occasion) {
      status.textContent = "Please select your event.";
      status.className = "venue-quote-status error";
      return;
    }
    ...
    const requirements = [
      ...
      "Submitted from venue profile quick quote"
    ].filter(Boolean).join("\n");

    button.disabled = true;
    button.textContent = "Sending your enquiry…";
    status.textContent = "";
    status.className = "venue-quote-status";
    ...
    byId("venueShareBtn")?.addEventListener("click", async () => {
      ...
      try {
        if (navigator.share) await navigator.share(shareData);
          ...
          byId("venueShareBtn").textContent = "Link Copied";
          setTimeout(() => byId("venueShareBtn").textContent = "Share", 1600);
        }
      } catch (_) {}
    });
