(function(){
  ...
  const compareParam=String(params.get("compare")||"").trim();
  ...
  function render(){
    ...
    grid.innerHTML=filtered.map(item=>{
      ...
      return`<article class="venue-card" data-venue-id="${escapeHtml(rawId)}" data-profile-url="${escapeHtml(profile)}" role="link" tabindex="0" aria-label="Open ${escapeHtml(name)} venue profile"><div class="venue-card-media">${media}<div class="venue-card-top"><span class="venue-type">${escapeHtml(item.venue_type||"Venue")}</span><span class="verified">✓ Verified</span></div></div><div class="venue-card-body"><h2>${escapeHtml(name)}</h2><p class="venue-location">⌖ ${locationText}</p>${fitHtml}<div class="venue-facts"><div class="venue-fact">${escapeHtml(capacity(item))}</div><div class="venue-fact">${escapeHtml(price(item))}</div></div><div class="venue-feature-list">${featureHtml}</div><p class="venue-description">${description}</p><div class="venue-actions"><a class="button button-primary" href="${profile}">View Venue</a><a class="button button-secondary" href="${quote}">Check Availability</a></div><div class="smv-venue-tools"><button class="smv-tool-btn ${shortlisted?"active":""}" type="button" data-shortlist="${escapeHtml(rawId)}">${shortlisted?"✓ Shortlisted":"♡ Shortlist"}</button><button class="smv-tool-btn ${compared?"active":""}" type="button" data-compare="${escapeHtml(rawId)}">${compared?"✓ Comparing":"Compare"}</button></div></div></article>`;
    }).join("");
    updateMarketBar();
  }
  ...
  function openMultiEnquiry(){
    ...
    form?.addEventListener("submit",async event=>{
      ...
      const {error}=await client.from("customer_enquiries").insert({customer_name:name,mobile,location:requirement.location||rows[0]?.city||null,occasion,event_date:date,guests,budget_per_person:budget,contact_remark:other||null,requirements,source:"Website - Multi Venue Enquiry",status:"new"});
  ...
  async function load(){
    ...
    if(compareParam&&venues.some(v=>String(v.id)===compareParam)&&!compare.includes(compareParam)){compare=[compareParam,...compare].slice(0,3);writeStoredIds(COMPARE_KEY,compare)}
  ...
  grid?.addEventListener("click",event=>{
    ...
    if(shortButton){event.preventDefault();event.stopPropagation();toggleShortlist(String(shortButton.dataset.shortlist||""));return}
    const compareButton=event.target.closest("[data-compare]");
    if(compareButton){event.preventDefault();event.stopPropagation();toggleCompare(String(compareButton.dataset.compare||""));return}
    if(event.target.closest("a,button,input,select,textarea,label"))return;
    const card=event.target.closest(".venue-card[data-profile-url]");
